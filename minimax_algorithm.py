            ...
            for col in range(board.size):
                if board.board[row][col] == ' ':
                    if col <= 5 and board.board[row][col + 1] == board.board[row][col + 2] != ' ':
                        score += TWO_IN_ROW_SCORE if board.board[row][col + 1] == 'O' else TWO_IN_ROW_OPPONENT_SCORE
                    if row <= 5 and board.board[row + 1][col] == board.board[row + 2][col] != ' ':
                        score += TWO_IN_ROW_SCORE if board.board[row + 1][col] == 'O' else TWO_IN_ROW_OPPONENT_SCORE

                    # Vérifier les alignements diagonaux
                    if row <= 5 and col <= 5 and board.board[row + 1][col + 1] == board.board[row + 2][col + 2] != ' ':
                        score += TWO_IN_ROW_SCORE if board.board[row + 1][col + 1] == 'O' else TWO_IN_ROW_OPPONENT_SCORE
